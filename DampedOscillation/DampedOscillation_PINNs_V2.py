...
layers = np.array([1,50,100,200,100,50,1])
...
#bounding conduction
def f_BC_1(self,x):
    return self.forward(x) - torch.ones_like(x).to(device)*x_0
...
def f_BC_2(self,x):
    ...
    return f_x - torch.ones_like(f_x).to(device)*v_0
...
class FCN(nn.Module):
    ...
    def lossBC(self,x_BC):
        ...
        loss_BC_1 = self.loss_function(f_BC_1(self,x_BC),torch.zeros_like(x_BC))
        loss_BC_2 = self.loss_function(f_BC_2(self,x_BC),torch.zeros_like(x_BC))
...
xh = torch.linspace(0,20,100).view(-1,1)
...
f_x = autograd.grad(f,g,torch.ones_like(g))[0]
